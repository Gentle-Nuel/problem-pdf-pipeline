...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (unclustered.length === 0) {
    return res.status(200).json({ ok: true, message: "Nothing new to cluster." });
  }
  ...
  // 4. Greedy assignment: compare each new item against the pool (existing
  // clusters + any created earlier in this same run), attach to the best
  // match above threshold, else start a new cluster and add it to the pool
  // so later items in the batch can match against it too.
  ...
  return res.status(200).json({
    ...
    newClusters: pool.filter((c) => c.isNew).length,
    touchedClusters: touchedPoolIndices.size,
  });
}
