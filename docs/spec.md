...
create table problem_clusters (
  ...
  status text default 'discovered',  -- discovered -> approved -> researched -> drafted -> approved_for_publish -> published (or researched -> blog_only, a terminal branch for research too thin to be a paid product — see "Companion blog post pipeline")
...
5. **Draft + generate PDF** — research is structured into a guide and rendered to PDF. Section headers aren't fixed to "problem/causes/fix" — the research prompt (`lib/gemini.ts`) lets the model choose headers that fit the actual question (a troubleshooting problem and a factual "what's the first X" question don't fit the same shape), always ending in a Resources section. Content runs through the humanize pass (`lib/humanize.ts`) before rendering. Template includes the standard disclaimer boilerplate. **Not every researched cluster becomes a PDF** — `lib/pricing.ts`'s `meetsMinimumDepthForPdf` gates this on the same word/source-count signal already used for pricing tiers (≥400 words, ≥1 source); anything under that routes to `status = 'blog_only'` instead, skipping PDF generation entirely. Confirmed live this matters: a real ~250-300 word research doc became a $5 PDF barely more substantial than its own free companion post — a bad product regardless of how the blog copy was worded, not something fixable by better wording alone.
...
1. **Generate the source content once, humanized** — `lib/humanize.ts` runs against the full `research_docs` content in `lib/generatePdfs.ts`, for every researched cluster regardless of whether it ends up with a PDF, and the result is persisted to `research_docs.humanized_content`. Strips AI-writing tells before anything publishes: no em dashes, no "it's not just X, it's Y" constructions, no inflated-significance language ("stands as a testament to..."), varied sentence length and rhythm, at least one specific/concrete detail instead of generic claims, and a real opinion or aside rather than flat neutral reporting. Stylistic only — see Guardrails above on the boundary against fabricated experience/credentials. This matters for reader trust as much as SEO — content that reads as obviously AI-generated undermines the "genuinely useful" premise the whole funnel depends on.
2. **Derive the blog post — no separate generation, no LLM call.** `lib/excerpt.ts` builds the post deterministically from the same humanized content: for a `blog_only` cluster (research too thin for a paid product, see Pipeline stages step 5), the post is the humanized content in full, no CTA, nothing held back — there's nothing to sell for that topic. For a cluster with a PDF, the post is a strict subset: the title plus the first `##` section, verbatim (capped at half the total word count if that first section alone would exceed it, cutting at the nearest paragraph or sentence boundary), followed by a programmatic CTA listing the real remaining section names — never an LLM-invented description of what's coming.

   This replaced an earlier design where the blog post was independently drafted by its own Gemini call. Confirmed live that let the draft (a) cover as much ground as a genuinely thin PDF, leaving nothing exclusive behind the paywall, and (b) invent CTA promises ("a complete chronological breakdown...") the PDF didn't actually deliver — a trust problem once real sales are involved. Deriving the post as a literal subset of the same already-humanized PDF content makes both problems structurally impossible rather than relying on prompt wording to avoid them, and removes an entire Gemini call per cluster in the process.
