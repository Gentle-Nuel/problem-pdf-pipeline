...
**Second, deeper fix on top of that (same day):** the prompt-only fix above stops the symptom but not the actual problem — a $5 PDF with ~250-300 words is a bad product no matter how the free post is worded. `lib/generatePdfs.ts` now gates PDF generation on `lib/pricing.ts`'s `meetsMinimumDepthForPdf` (≥400 words, ≥1 source — reusing the existing $9-tier floor rather than inventing a new number); anything under that routes the cluster to `status = 'blog_only'` instead of forcing a paid product to exist. A `blog_only` cluster still gets a full free post (`lib/blogDraft.ts` writes a complete standalone answer with no guide reference, instead of a teaser — there's nothing to point to), reviewed and published exactly like any other post, it just never gets a `pdfs` row, a PDF review tap, or a Gumroad handoff. `lib/reviewBlogPosts.ts`'s Telegram message now notes when a post has no paid guide behind it, so this doesn't look like a missing step. No new migration needed — `problem_clusters.status` is a plain text column, `blog_only` is just a new value in it. Not yet re-tested live — needs a fresh cluster through the whole chain, since the already-published test posts predate both fixes.
