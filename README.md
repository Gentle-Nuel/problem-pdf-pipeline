...
**2026-07-24 — first real Gumroad listing published.** The pipeline's first actual product went live on Gumroad from a post-wipe run — the "someone asked this on Stack Exchange" → "priced guide for sale" loop closed for real, not just in testing.

**2026-07-24 — Telegram `/scrape` and `/cluster` commands.** The bot now runs the same logic as the cron-triggered endpoints on demand from a chat message, restricted to the configured `TELEGRAM_CHAT_ID` — no need to open a browser to trigger a manual run. See `docs/spec.md`'s Tech stack section for the response-timing detail that makes this safe against Telegram's webhook retry behavior.
...
1. **Derive the blog post — as of 2026-07-24, zero Gemini calls here** (`lib/generateBlogPosts.ts`) — picks up clusters ready for a companion post (`status in ('drafted', 'blog_only')`, `problem_clusters.blog_generated_at is null`, capped at `BLOG_PER_RUN` — `lib/config.ts`, starts at 3) and derives the post deterministically from `research_docs.humanized_content`, which `lib/generatePdfs.ts` already computed and persisted for every researched cluster regardless of whether it got a PDF. See "Rearchitected" below for why this replaced an independent Gemini-drafted post.

   Both `draft_content` and `final_content` get the same derived text (`status = 'humanized'`) — there's no separate humanize stage for the blog anymore, the content was already humanized upstream.
...
**Real trust-gap caught after the site went live with real posts (2026-07-24):** for a thin-research question ("What's the first game with an all-female cast?"), the blog draft's CTA promised "a complete, chronological breakdown... from the earliest days of the arcade up through the golden age of home computers and RPGs" — but the actual PDF was a short "Short Answer" plus a couple of bullet points, nowhere near that promise. The blog draft is generated independently of the final PDF from the same research, so nothing was constraining it to (a) actually leave more in the PDF than it covered itself, or (b) avoid inventing CTA promises the research didn't support — for a thin-research topic, the free teaser could end up more substantial than the paid guide, inverted from the intended value proposition. `lib/blogDraft.ts`'s prompt now explicitly requires the PDF to always read as more substantial (shrink the teaser for thin research instead of padding it to a length target) and bars the CTA from promising specific content beyond what the research actually supports.

**Second, deeper fix on top of that (same day):** the prompt-only fix above stops the symptom but not the actual problem — a $5 PDF with ~250-300 words is a bad product no matter how the free post is worded. `lib/generatePdfs.ts` now gates PDF generation on `lib/pricing.ts`'s `meetsMinimumDepthForPdf` (≥400 words, ≥1 source — reusing the existing $9-tier floor rather than inventing a new number); anything under that routes the cluster to `status = 'blog_only'` instead of forcing a paid product to exist. `problem_clusters.status` is a plain text column, so `blog_only` needed no migration.

**Rearchitected, same day, after confirming `blog_only` worked live:** the two fixes above were both patches on a design where the blog post was independently drafted by its own Gemini call (`lib/blogDraft.ts`). Rather than keep patching that call's prompt, replaced it entirely: `lib/generatePdfs.ts` now humanizes `research_docs.content` once for *every* researched cluster (whether or not it clears the depth gate) and persists the result to a new `research_docs.humanized_content` column (`supabase/migrations/0011_research_docs_humanized_content.sql`). `lib/generateBlogPosts.ts` derives the post from that same text with **no LLM call at all**: a `blog_only` cluster gets the humanized content in full; a cluster with a PDF gets a deterministic excerpt (`lib/excerpt.ts` — the title plus the first `##` section, verbatim, capped at half the total word count with a paragraph/sentence-level fallback if the first section alone would exceed that) plus a **programmatic** CTA that lists the real remaining section names — never an LLM guessing what to promise. `lib/blogDraft.ts` deleted; `lib/humanize.ts` now has exactly one call site (`lib/generatePdfs.ts`) instead of two.

This makes both earlier problems structurally impossible instead of prompt-discouraged: the blog can't cover more than the PDF (it's a literal subset), and the CTA can't promise anything that isn't a real section in the document. Verified the excerpt/CTA logic directly with `npx tsx` against synthetic content matching both research shapes (troubleshooting-style and factual-style) before deploying — caught and fixed a real bug in the process: a single dense section with no internal paragraph breaks defeated the word-count safety bound entirely (nothing to cut at), fixed with a sentence-level fallback, then reverified.

**To wire it up:**
1. Run `supabase/migrations/0008_blog_posts_tracking.sql` and `supabase/migrations/0011_research_docs_humanized_content.sql` in the Supabase SQL editor (`blog_posts` table itself already existed from `0001_init.sql`).
2. No new API key needed.
3. Push/redeploy, then trigger `/api/cluster-problems?secret=<CRON_SECRET>` — response includes `drafted`, `routedBlogOnly`, `blogDrafted`, and `blogReviewSent` counts.
4. Check the `blog_posts` table in Supabase for a new row with `draft_content`/`final_content` populated, and `pdf_id` either set (excerpt-style post) or null (`blog_only`, full post).
5. Check Telegram for a preview message with a word count and an "✅ Approve" button — should note "No paid guide for this one" if `pdf_id` is null.
...
**Two real issues caught from reading a real live page after the blog excerpt rearchitecture (2026-07-24):**
- **`meta description` and `og:description` were shipping the literal string "Short Answer"** (or whatever the post's first section happened to be called) on every single page — a real regression from the rearchitecture, not pre-existing. `site/src/lib/postMeta.ts`'s `extractDescription` assumed the content right after the title was flowing prose (true of the old independently-drafted blog post), but excerpt-based posts always start with an isolated `## Header` line before any prose — "split on blank lines, take the first chunk" grabbed the header line itself. Fixed to skip header-only chunks and find the first real paragraph. This was silently undermining the site's entire SEO purpose on every page — worth catching before more accumulate. Verified the fix directly with `npx tsx` against realistic excerpt content before shipping.
- **The disclaimer read oddly specific on benign topics.** The original wording asserted the guide "involved electrical, structural, plumbing... or health, legal, or financial matters" — accurate framing for diy/cooking, but odd on something like "how to check if film is loaded in the dark" from the photo site. Reworded (`lib/disclaimer.ts` and `site/src/lib/disclaimer.ts`, kept in sync) from an assertion to a conditional ("if anything here touches..."), so it fits every topic without needing per-cluster logic to pick between variants.

Both fix retroactively on the next site rebuild — Astro rebuilds every page from the database each time, nothing needs a per-page backfill.
